...
from PIL import Image , ImageTk   #Importation de la Bibliothèque pillow pour inserer les images dans la fenêtre
import threading
import time
...
def calculerDiviseurs(nombre, listbox, update_interval=0.1):
    listeDesDiviseurs = []
    if nombre.isnumeric():
        nombre = int(nombre)
...
def calculDiviseur():
    ...
    thread = threading.Thread(target=calculerDiviseurs, args=(nombre, listbox))
    thread.start()

    # Réactiver les boutons après le calcul
    fenetre.after(0, boutonCalculer.config, {"state": NORMAL})
    fenetre.after(0, boutonAnnuler.config, {"state": NORMAL})
